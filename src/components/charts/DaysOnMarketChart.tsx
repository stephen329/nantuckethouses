...
export function DaysOnMarketChart({ data, isLoading }: Props) {
  ...
  if (!data || data.length === 0) {
    return (
      <div className="h-[380px] bg-white rounded-sm border border-[#6B5E51] p-6">
        <h3 className="text-[#1B263B] text-lg font-medium mb-4">Days on Market Trend</h3>
        <div className="h-[300px] flex items-center justify-center bg-[#F1F1E6] rounded border border-dashed border-[#6B5E51]">
          <p className="text-[#1B263B]/50 text-sm">No days on market data available</p>
        </div>
      </div>
    );
  }
  ...
  if (!hasValidData) {
    return (
      <div className="h-[380px] bg-white rounded-sm border border-[#6B5E51] p-6">
        <h3 className="text-[#1B263B] text-lg font-medium mb-4">Days on Market Trend</h3>
        <div className="h-[300px] flex flex-col items-center justify-center bg-[#F1F1E6] rounded border border-dashed border-[#6B5E51]">
          <p className="text-[#1B263B]/50 text-sm">Insufficient sample size for median calculation</p>
          <p className="text-[#1B263B]/40 text-xs mt-1">DOM data requires completed sales transactions</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-sm border border-[#6B5E51] p-6">
      <div className="flex items-start justify-between mb-4">
        <div>
          <h3 className="text-[#1B263B] text-lg font-medium">Days on Market Trend</h3>
          {marketCondition && (
          ...
          <AreaChart data={chartData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
            <defs>
              <linearGradient id="domGradient" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor="#2D4635" stopOpacity={0.4}/>
                <stop offset="95%" stopColor="#2D4635" stopOpacity={0.05}/>
              </linearGradient>
            </defs>
              ...
              content={({ active, payload, label }) => {
                ...
                return (
                  <div className="bg-white border border-[#6B5E51] rounded p-2 text-sm shadow-md">
                    <p className="font-semibold text-[#1B263B] mb-1">
            ...
            {avgDOM && (
              <ReferenceLine 
                ...
                stroke="#1B263B" 
                strokeDasharray="5 5"
                label={{ value: "Avg", position: "right", fontSize: 9, fill: "#1B263B" }}
              />
            )}
            <Area 
              type="monotone" 
              ...
              dot={false}
              activeDot={{ r: 5, fill: "#2D4635", stroke: "#fff", strokeWidth: 2 }}
              connectNulls
            />
          </AreaChart>
        </ResponsiveContainer>
      ...
      <div className="mt-4 pt-3 border-t border-[#E8E8E8] text-xs text-[#1B263B]/60">
